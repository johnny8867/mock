class Trie:
    ...
    def pruneWord(self, word):
        cur: Trie = self
        node_childkey: list[tuple[Trie, str]] = []
        for char in word:
            node_childkey.append((cur, char))
            cur = cur.contain[char]
            
        for parentNode, childKey in reversed(node_childkey):
            targetNode = parentNode.contain[childKey]
            if len(targetNode.contain) == 0:
                del parentNode.contain[childKey]
            else:
                return
        ...
        def dfs(r,c, root, word):
            if 0 <= r < len(board) and 0 <= c < len(board[0]) and (r, c) not in visited and board[r][c] in root.contain:
                ...
                if root.end:
                    result.add(word)
                    root.end = False
                    root_list.pruneWord(word) #need this to pass tc
